...
#使用只使用40个州特征和2个阳性测试特征（索引57和75）
class COVIDRegressionDataset_2(Dataset):
    def __init__(self, path, transform=None, normalize=True,
                 feature_scaler=None, target_scaler=None):
        """
        COVID 回归数据集
        ...
        # 分离特征和目标
        selected_columns = list(range(40)) + [58, 76]
        self.features = self.df.iloc[:, selected_columns].values.astype(np.float32)
        self.targets = self.df.iloc[:, -1].values.astype(np.float32)

        # 可选：查看所有选中列的列名
        print("所有选中的列名:")
        for idx in selected_columns:
            print(f"列{idx}: {self.df.columns[idx]}")
...
#使用只使用40个州特征和5个计算得到的最好（索引76,78,68,83,80）
class COVIDRegressionDataset_3(Dataset):
    def __init__(self, path, transform=None, normalize=True,
                 feature_scaler=None, target_scaler=None):
        """
        COVID 回归数据集
        ...
        # 分离特征和目标
        selected_columns = list(range(40)) + [78, 76, 68, 83,80]
        self.features = self.df.iloc[:, selected_columns].values.astype(np.float32)
        self.targets = self.df.iloc[:, -1].values.astype(np.float32)
